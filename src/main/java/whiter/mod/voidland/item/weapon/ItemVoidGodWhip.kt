...
@RegisterItem
class ItemVoidGodWhip: Item() {
    val peachMaterial = EnumHelper.addToolMaterial("VOIDGODWHIP", 3, 2420, 16.0F, 0.0F, 10);
    var attackDamage: Float = -1.0F
    init {
        this.maxStackSize = 1
        this.maxDamage = 2420
        //this.creativeTab = CreativeTabs.COMBAT
        this.attackDamage = 26.0f + attackDamage
    }

    @SideOnly(Side.CLIENT)
    override fun addInformation(stack: ItemStack?, @Nullable player: World?, tooltip: MutableList<String>, advanced: ITooltipFlag?) {
        tooltip.add("传说是当年姜子牙使用的武器，此鞭有二十六节每结上有四道符印‖手持无视怪物的克制效果")
    }
